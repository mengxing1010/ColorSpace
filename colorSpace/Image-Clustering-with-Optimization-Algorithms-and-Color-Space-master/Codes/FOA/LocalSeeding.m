...
for q=1:size(Forest.T,1)
    if Forest.T(q,Forest.P.Dimension+2)==0 %Dimension+2 shows the Age of each tree
        current=Forest.T(q,:);
        Forest.T(q,1:Forest.P.Dimension) = round(Forest.T(q,1:Forest.P.Dimension));
        ...
        for ww=move
            ...
            temp(1,ww)=round(temp(1,ww)+add);
            if temp(1,ww)<Forest.P.Llimit
                temp(1,ww)=Forest.P.Llimit + 2;
            elseif temp(1,ww)>Forest.P.Ulimit
                temp(1,ww)=Forest.P.Ulimit - 2;
            end
